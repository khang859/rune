...
Use `/plan` or cycle with `Shift+Tab` to enter Plan Mode. While active, write/edit/bash and shell shortcuts are disabled; the read-only `gh` tool remains available for GitHub inspection. MCP tools are available only when marked read-only or allowlisted with `read_only` / `plan_tools` in MCP config. Use `/approve`, `/act`, or continue cycling with `Shift+Tab` to return to Act Mode before implementation. See `docs/plan-mode.md`.
