...
func main() {
	flag.Usage = func() {
		fmt.Fprintln(os.Stderr, "usage: rune [--provider codex|groq|ollama|runpod] [--model <id>] [--prompt <text>] [--version] | rune login codex | rune mcp <command>")
		flag.PrintDefaults()
	}
	...
	provider := flag.String("provider", "", "provider id (codex, groq, ollama, or runpod; overrides RUNE_PROVIDER and settings)")
