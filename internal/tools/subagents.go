...
func (SpawnSubagent) Spec() ai.ToolSpec {
	return ai.ToolSpec{Name: "spawn_subagent", Description: "Start a specialized subagent with isolated context. After spawning, end your turn — the system resumes you and injects the summary when the subagent finishes; do not poll with get_subagent_result. When to use each type: code-explorer: investigating an unfamiliar feature area (>3 file reads or broad grep). code-architect: designing a non-trivial feature touching multiple files. code-reviewer: verifying a substantial change before reporting it complete. exploration: broader read-only discovery. validator: sanity-checking a plan before implementation. general: catch-all for self-contained delegated work. Custom subagents from ~/.rune/agents and ./.rune/agents are also available.", Schema: json.RawMessage(`{"type":"object","properties":{"name":{"type":"string"},"prompt":{"type":"string"},"agent_type":{"type":"string","default":"general","description":"Subagent type/name. Available built-ins: general, exploration, validator, code-explorer, code-architect, code-reviewer. Custom types can be declared in ~/.rune/agents/*.md or ./.rune/agents/*.md."},"background":{"type":"boolean","default":true},"dependencies":{"type":"array","items":{"type":"string"},"description":"Optional subagent task IDs that must complete before this task starts."},"timeout_secs":{"type":"integer","default":600}},"required":["name","prompt"]}`)}
}
