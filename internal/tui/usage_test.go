...
func TestFormatUsageStats_FullAndEmpty(t *testing.T) {
	...
	for _, want := range []string{"model=gpt-5", "effort=high", "turns=2", "total 440", "output 25.0 tok/s", "subagents=1", "created"} {
	...
	if !strings.Contains(ej, "effort=—") || !strings.Contains(ej, "turns=0") || !strings.Contains(ej, "output —") {
