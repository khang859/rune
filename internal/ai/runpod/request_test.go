...
func TestBuildPayload_OmitsReasoningEffort(t *testing.T) {
	b, err := buildPayload(ai.Request{Model: "openai/gpt-oss-120b", Reasoning: ai.ReasoningConfig{Effort: "medium"}})
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(string(b), "reasoning_effort") {
		t.Fatalf("reasoning_effort should be omitted for Runpod:\n%s", b)
	}
}
